...
const output = path.resolve(process.argv[3] || "tmp/cad-cae-weekly-qa");
...
async function assertArticle(page, label) {
  ...
  for (const text of ["本周最优先的 3 项行动", "AI × CAD/CAE", "决策与监控清单", "来源与证据边界"]) {
    if ((await page.getByText(text, { exact: false }).count()) === 0) throw new Error(`${label}: missing ${text}`);
  }
  if ((await page.locator("#article-container table").count()) < 3) {
    throw new Error(`${label}: expected at least three engineering tables`);
  }
  ...
  const overflow = await page.evaluate(() => {
    ...
    return article && article.scrollWidth > article.clientWidth + 1;
  });
  if (overflow) {
    ...
    throw new Error(`${label}: horizontal page overflow ${JSON.stringify(offenders)}`);
  }
}
...
async function main() {
  ...
  await settle(page);
  if ((await page.locator(`a[href="/p/${latest.postId}.html"]`).count()) === 0) throw new Error("Homepage missing latest CAD/CAE weekly briefing");
  const background = await page.locator(".category-cad-cae").evaluate((element) => getComputedStyle(element).backgroundImage);
  if (!background.includes(path.basename(latest.heroImage))) throw new Error("CAD/CAE category card has no latest local cover");
  ...
  await page.goto(`${origin}/categories/${encodeURIComponent("CAD-CAE生态周报")}/`, { waitUntil: "domcontentloaded" });
  await settle(page);
  if ((await page.locator(`a[href="/p/${latest.postId}.html"]`).count()) === 0) throw new Error("CAD/CAE category missing latest article");
